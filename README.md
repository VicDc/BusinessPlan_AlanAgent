...
The **`local` provider (LM Studio / Ollama) is the only one that keeps everything on your machine**, and it's the only provider that is actually tested. The only outbound traffic in that mode is web search for market data and grants. In practice this is served by `ddgs`, which is free and needs no API key. The Serper and Playwright tiers exist in the code but are **not currently active** (Serper needs a paid key, Playwright is not installed). This traffic must never include sensitive founder data.
...
| **Web search** | ddgs (free, no API key). Serper and Playwright tiers implemented but not active |
...
```powershell
git clone https://github.com/VicDc/business-plan-ai.git
cd business-plan-ai
...
python -m scripts.intake_cli run
...
Il **provider `local` (LM Studio / Ollama) è l'unico che tiene tutto sulla tua macchina**, ed è l'unico effettivamente testato. L'unico traffico in uscita in quella modalità è la ricerca web per dati di mercato e bandi. In pratica è servita da `ddgs`, gratuita e senza API key. I livelli Serper e Playwright esistono nel codice ma **non sono attualmente attivi** (Serper richiede una key a pagamento, Playwright non è installato). Questo traffico non deve mai contenere dati sensibili sui soci.
...
| **Ricerca web** | ddgs (gratuita, nessuna API key). Livelli Serper e Playwright implementati ma non attivi |
